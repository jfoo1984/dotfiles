...
Use `prompts/implementer.md` for the implementer prompt. Reviewers are dispatched using lenses from the shared lens library at `.claude/skills/review-lenses/lenses/` — they bake in the discipline that matters.
...
4. **Dispatch code-quality reviewer** via Agent tool with `subagent_type: feature-dev:code-reviewer`, prompt from `.claude/skills/review-lenses/lenses/code-quality.md`. Fill the lens's slot headings: **Target** (diff + file paths), **What was implemented** (goal from the plan), **Already verified** (implementer's verify output). Apply blocking + important fixes per decision rules.
5. **Dispatch staff-engineer reviewer** via Agent tool with `subagent_type: feature-dev:code-reviewer`, prompt from `.claude/skills/review-lenses/lenses/staff-engineer.md`. Fill the same slots; pass the code-quality verdict under **Already verified** so it isn't re-litigated. Same fix-apply pattern. Forward-looking concerns often belong in "open follow-ups" rather than the current task.
...
Reviewer prompts are provided by the shared lens library at **`.claude/skills/review-lenses/`** rather than carried in this skill — this keeps lens evolution centralized so improvements benefit all consumers (this skill, `review-code`, the PR bot).

Use **superpowers-extended-cc:subagent-driven-development** if you want spec compliance as a separate gate. Use **superpowers-extended-cc:writing-plans** to author the plan this skill executes against.

## Retirement note

The global `~/.claude/skills/implement-task-with-review` should be retired once this in-repo version is confirmed working, to avoid name-shadowing between the global and project skill. Retirement is handled as a follow-up task (see the branch's task list).
