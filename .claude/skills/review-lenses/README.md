...
2. **Impact / blast radius** — trace the callers, implementers, and dependents the change touches and check they still hold. An interface or schema change frequently breaks code that isn't in the diff.
...
```yaml
subagent_type: superpowers-extended-cc:code-reviewer
```

If that agent isn't available in the current environment, `general-purpose` works — the lens file carries the whole persona, so the agent type only needs read/search tools.

The model default is stated in each lens file. Consumers may override it, but should prefer the lens-file default: `sonnet` for code-quality (rarely needs more), `opus` for staff-engineer, adversarial, and security (judgment benefits from the stronger model).

**Model defaults are tier aliases, not pinned versions.** `sonnet` / `opus` resolve to the current latest model in that tier, so they track model upgrades automatically — do not pin a dated id. Naming a tier (rather than omitting `model` to inherit the session model) keeps each role at its intended capability level across releases: a session running on sonnet would otherwise silently downgrade the staff-engineer/adversarial/security review. Revisit these defaults only if a tier is added or renamed.
...
These lenses are deliberately **project-portable**: they encode general software-engineering principles and name no framework, path, or house convention. Project-specific rules are injected by the *consumer*, which reads the repo's `CLAUDE.md` / `AGENTS.md` and passes the relevant rules as context (see `review-code` Step 3).

If a project accumulates enough of its own conventions to be worth a standing lens, add a **stack lens** beside these (e.g. `lenses/frontend.md`, `lenses/backend-go.md`) in that project's own `.claude/skills/`, not here. Stack lenses cite the project's convention docs as their authority; the lenses in this library must stay stack-neutral.

---

## Keeping lenses current

Lenses are living. Improve one on **evidence from a review** — a real miss or a repeated false positive — not on speculation. **Keep every edit as terse as the lens itself**: each line is re-sent to the reviewer on every dispatch, so add only necessary info and prefer compressing or replacing over appending — a lens that only grows loses focus.

Detection is the **consumer's** job, not the lens's: a single-diff lens subagent can't tell a one-off from a recurring gap, so the consumer surfaces a one-line **lens-gap note** when it spots one. Lenses stay pure reporters — they never edit themselves.
